...
export const PartyLedgerPrintModal: React.FC<PartyLedgerPrintModalProps> = ({
  ...
  // Auto-scale on mobile so the A4 statement fits horizontally on iPhone / mobile screens
  useEffect(() => {
    const handleResize = () => {
      if (typeof window !== 'undefined') {
        if (window.innerWidth < 768) {
          const targetWidth = 760; // standard width of printable A4 document container
  ...
  // Compute chronologically sorted entries with running balance
  // Sorted oldest to newest for the formal statement
  ...
  const handleCopySummary = () => {
    const text = `*Party Ledger Statement: ${customer.name}*\n` +
      `Issued By: ${clubProfile.businessName}\n` +
      `Date: ${new Date().toLocaleDateString('en-IN')}\n` +
      `Total Billed (Debit): Rs. ${totalDebits.toLocaleString('en-IN')}\n` +
      `Total Paid (Credit): Rs. ${totalCredits.toLocaleString('en-IN')}\n` +
      `Closing Balance Due: Rs. ${Math.abs(netClosingBalance).toLocaleString('en-IN')} ${isDebitBalance ? 'DR' : 'CR'}\n` +
      `UPI Pay ID: ${clubProfile.upiId}`;
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="truncate">
              ...
              <p className="text-[10px] text-slate-400 font-mono">
                {entries.length} Transaction(s) • Balance: ₹{Math.abs(netClosingBalance).toLocaleString('en-IN')} {isDebitBalance ? 'DR' : 'CR'}
              </p>
            </div>
          </div>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    ...
                    {processedEntries.length === 0 ? (
                      ...
                      processedEntries.map((entry) => {
                        ...
                        const badge = entry.type === 'DEBIT_SESSION' 
                          ? 'GAME' 
