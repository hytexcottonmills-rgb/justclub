...
import { checkSlugAvailability, registerTenantSlug, getClubSlug } from '../utils/payToken';
...
export const SetupConfigView: React.FC<SetupConfigViewProps> = ({
  ...
  const currentSlugInput = profileForm.paymentSlug || getClubSlug({ businessName: profileForm.businessName, upiId: profileForm.upiId });
  const slugStatus = useMemo(() => {
    return checkSlugAvailability(
      currentSlugInput,
      profileForm.id || 'current_club',
      profileForm.pincode,
      profileForm.businessName
    );
  }, [currentSlugInput, profileForm.id, profileForm.pincode, profileForm.businessName]);

  const handleProfileSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!slugStatus.isAvailable && currentSlugInput.trim()) {
      alert(`The slug '${currentSlugInput}' is unavailable or taken by another club. Please pick an available handle from the suggestions.`);
      return;
    }
    const finalSlug = currentSlugInput.trim().toLowerCase().replace(/[^a-z0-9_-]/g, '');
    const updatedProfile = { ...profileForm, paymentSlug: finalSlug };
    registerTenantSlug(updatedProfile.id, finalSlug, updatedProfile.businessName, updatedProfile.upiId);
    onUpdateClubProfile(updatedProfile);
              ...
              <div className={`pt-3 border-t grid grid-cols-1 sm:grid-cols-2 gap-4 ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className={`font-bold block ${isDarkMode ? 'text-indigo-300' : 'text-indigo-700'}`}>
                    ...
                    {slugStatus.isAvailable ? (
                      ...
                      <span className="text-[10px] font-bold text-rose-400 bg-rose-950/80 border border-rose-800/80 px-2 py-0.5 rounded-md flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" />
                        {slugStatus.isTakenByOther ? 'Claimed' : 'Reserved'}
                      </span>
                    )}
                  </div>
                  ...
                  {!slugStatus.isAvailable && (
                    <div className="mt-2 p-2.5 rounded-xl bg-rose-950/40 border border-rose-800/50 text-[11px] space-y-1.5">
                      <p className="text-rose-300 font-semibold flex items-center gap-1">
                        <XCircle className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                        {slugStatus.isTakenByOther
                          ? `The handle '${currentSlugInput}' is taken by another registered club.`
                          : `The handle '${currentSlugInput}' is reserved by the system.`}
                      </p>
                      {slugStatus.suggestions.length > 0 && (
                        <div>
                          <p className="text-slate-300 font-medium mb-1">Click an available suggestion to apply:</p>
                          <div className="flex flex-wrap gap-1.5">
                            {slugStatus.suggestions.map((sugg) => (
                              <button
                                key={sugg}
                                type="button"
                                onClick={() => setProfileForm({ ...profileForm, paymentSlug: sugg })}
                                className="px-2.5 py-1 rounded-lg bg-indigo-900/90 hover:bg-indigo-600 text-indigo-200 hover:text-white border border-indigo-700/80 transition font-mono font-bold text-xs shadow-sm flex items-center gap-1"
                              >
                  ...
                  {slugStatus.isAvailable && (
                    <p className="text-[11px] text-slate-500 mt-1">
                      Custom short link for WhatsApp reminders: <code className="text-indigo-400 font-mono">justclub.in/p/{currentSlugInput}/2900</code>
                    </p>
                  )}
                </div>
              </div>
