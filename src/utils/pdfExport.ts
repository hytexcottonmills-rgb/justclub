...
/**
 * High-reliability PDF export for Party Ledgers and Invoices.
...
export async function downloadInvoiceAsPdf(
  ...
  sandbox.style.top = '0';
  sandbox.style.width = (elementId === 'printable-ledger' || elementId === 'printable-invoice' || elementId === 'printable-bill-record') ? '760px' : '800px';
  ...
  try {
    // Wait for any embedded images (logos, stamps) to be fully ready
    ...
    for (let i = 0; i < elementsToCapture.length; i++) {
      ...
      const canvas = await html2canvas(pageEl, {
        scale: 2, // 2x for crisp text and numbers
        useCORS: true,
        allowTaint: true,
        logging: false,
        ...
        windowWidth: 850,
        onclone: (clonedDoc) => {
          // Sanitize CSS rules for modern color functions unsupported by html2canvas
      ...
      // If single long multi-page ledger, paginate cleanly across A4 sheets
...
/**
 * Universal Print Helper for Party Ledgers and Invoices.
...
export async function printDocumentElement(elementId: string): Promise<boolean> {
  ...
  if (isIOS()) {
    const existingRoot = document.getElementById('justclub-print-root');
    if (existingRoot && existingRoot.parentNode) {
      existingRoot.parentNode.removeChild(existingRoot);
    }

    const printRoot = document.createElement('div');
    printRoot.id = 'justclub-print-root';
    printRoot.className = 'justclub-print-root';
    ...
    document.body.classList.add('justclub-printing');
    ...
    try {
      ...
      setTimeout(() => {
        document.body.classList.remove('justclub-printing');
  ...
  try {
    const oldIframe = document.getElementById('justclub-print-iframe');
    if (oldIframe && oldIframe.parentNode) {
      oldIframe.parentNode.removeChild(oldIframe);
    }

    const printIframe = document.createElement('iframe');
    printIframe.id = 'justclub-print-iframe';
    ...
    const printStyles = `
      <style>
        ...
        *, *::before, *::after {
        ...
        #printable-ledger, .printable-document {
          visibility: visible !important;
